...
class ResourceManager:
    ...
    def _log_resource_stats(self, metrics: ResourceMetrics) -> None:
        ...
        try:
            from resource_alerts import evaluate_resource_alerts  # type: ignore
    ...
    def can_accept_job(self) -> bool:
        """
        Check if the server can accept new jobs based on resource availability.

        Returns:
            bool: True if server can accept new jobs, False otherwise
        """
        try:
            metrics = self.memory_manager.check_memory_usage()

            # Check memory thresholds
            memory_ok = metrics.memory_percent < 85.0  # Don't accept if > 85% memory

            # Check connection limits
            http_stats = self.http_pool.get_stats()
            connections_ok = http_stats.active_connections < self.resource_config.http_pool.max_pool_size * 0.9

            # Check CPU usage
            cpu_ok = metrics.cpu_percent < 90.0  # Don't accept if CPU > 90%

            can_accept = memory_ok and connections_ok and cpu_ok

            if not can_accept:
                logger.warning(
                    f"Cannot accept new job - Memory: {metrics.memory_percent:.1f}%, "
                    f"CPU: {metrics.cpu_percent:.1f}%, "
                    f"Connections: {http_stats.active_connections}/{self.resource_config.http_pool.max_pool_size}"
                )

            return can_accept
        except Exception as e:
            logger.error(f"Error checking job acceptance: {e}")
            # Be conservative - don't accept if we can't check
            return False
